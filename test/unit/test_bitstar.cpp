...
int main(int argc, char **argv) {
    ...
    */
    ...
    planner_params.setParam("num_batch", 200); // Adding samples (any time!)
    ...
    auto node = std::make_shared<rclcpp::Node>("bitstar_visualizer", rclcpp::NodeOptions().parameter_overrides({
    ...
    std::shared_ptr<Planner> planner = PlannerFactory::getInstance().createPlanner(PlannerType::BITStar, statespace,problem_def, obstacle_checker);
    ...
    while (running && rclcpp::ok()) {
        ...
        // std::vector<Eigen::VectorXd> shortest_path_ = dynamic_cast<BITStar*>(planner.get())->getSmoothedPathPositions(5, 2);
