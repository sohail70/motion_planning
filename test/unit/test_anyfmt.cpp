...
int main(int argc, char **argv) {
    ...
    */
    ...
    auto node = std::make_shared<rclcpp::Node>("anyfmt_visualizer", rclcpp::NodeOptions().parameter_overrides({
    ...
    std::shared_ptr<Planner> planner = PlannerFactory::getInstance().createPlanner(PlannerType::ANYFMT, statespace,problem_def, obstacle_checker);
    ...
    while (running && rclcpp::ok()) {
        ...
        // std::vector<Eigen::VectorXd> shortest_path_ = dynamic_cast<ANYFMT*>(planner.get())->getSmoothedPathPositions(5, 2);
