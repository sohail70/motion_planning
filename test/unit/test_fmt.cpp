...
int main(int argc, char **argv) {
    ...
    */
    ...
    // Create ROS node
    // auto node = std::make_shared<rclcpp::Node>("fmt_visualizer");
    auto node = std::make_shared<rclcpp::Node>("fmt_visualizer", rclcpp::NodeOptions().parameter_overrides({
    ...
    std::shared_ptr<Planner> planner = PlannerFactory::getInstance().createPlanner(PlannerType::FMT, statespace,problem_def, obstacle_checker);
    ...
    while (running && rclcpp::ok()) {
        ...
        // std::vector<Eigen::VectorXd> shortest_path_ = dynamic_cast<FMT*>(planner.get())->getSmoothedPathPositions(5, 2);
        // ros2_manager->followPath(shortest_path_);

        // dynamic_cast<FMT*>(planner.get())->visualizeSmoothedPath(shortest_path_);
        // dynamic_cast<FMT*>(planner.get())->visualizeHeapAndUnvisited();
