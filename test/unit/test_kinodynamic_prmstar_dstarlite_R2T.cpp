...
int main(int argc, char** argv)
{
    ...
    auto vis_node = std::make_shared<rclcpp::Node>("prm_visualizer",
    ...
    auto planner = PlannerFactory::getInstance().createPlanner(PlannerType::KinodynamicPRMStarDStarLite, statespace, problem_def, obstacle_checker);
    
    auto kinodynamic_planner = dynamic_cast<KinodynamicPRMStarDStarLite*>(planner.get());
    ...
    if (gazebo_checker) {
        ...
        std::cout << "[SYNC] PRM Timers initialized against frozen physics.\n";
    }
    ...
    while (g_running && rclcpp::ok()) 
    {
        ...
        //         // --- PRINT OUTPUT HERE ---
        // std::cout << "[DEBUG] Path Size: " << new_executable_path.size() << std::endl;
        // for (size_t i = 0; i < new_executable_path.size(); ++i) {
        //     std::cout << "Point " << i << ": [" 
        //               << new_executable_path[i].x() << ", " 
        //               << new_executable_path[i].y() << ", " 
        //               << new_executable_path[i].z() << "]" << std::endl;
        // }
        // // -------------------------
    ...
    std::string filename = "sim_prm_" + std::to_string(num_of_samples_val) + 
