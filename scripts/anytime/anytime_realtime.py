...
# ALGORITHMS = ["KinodynamicLLPTStar"]
...
def create_yaml(algo_name, space_name, seed, params, filename):
    ...
    config = {
        ...
        "simulation": {
            "slice_time": 0.02 if space_name != "R2" else 0.02,
    ...
    #     config["planner_params"]["heuristic"] = bool(params["heuristic"])
...
def run_experiments():
    ...
    for space_name, params in STATE_SPACES.items():
        for algo in ALGORITHMS:
            for seed in range(START_SEED, START_SEED + NUM_SEEDS):
                ...
                # Pin to cores 2,3 and give realtime priority 80
                # command = [
                #     "sudo", "chrt", "-f", "80",
                #     "taskset", "-c", "2,3",
                #     EXECUTABLE_PATH, temp_yaml_file
                # ]
    ...
    if ENABLE_AUTO_SHUTDOWN:
            ...
            # This will execute automatically if the experiments finish
            subprocess.run(SHUTDOWN_COMMAND, check=True)



# def set_cpu_performance():
#     """Set all CPU cores to the 'performance' governor (requires sudo)."""
#     try:
#         subprocess.run(
...
#         print("[WARN] Could not set CPU governor (run script with sudo).")
#     except FileNotFoundError:
#         print("[WARN] cpupower not found; skipping governor setup.")
