...
_BASE = "."   
CONFIGS = {
    "Dubins_m1": f"{_BASE}/results_samples_1_dubins",
    "Dubins_m2": f"{_BASE}/results_samples_2_dubins",
}
...
DOMAIN_METRIC = {"Dubins":   ("exec_turn",   "Turn (rad)"),
                 ...
                 "T_exec":   ("exec_time",   "Traversal time (s)") 
                 }


def fig_execution_time(tidy):
    ...
    for ax, scen in zip(axes[0], scs):
        _grouped_box(ax, tidy, scen, "exec_time", "Traversal time (s)")
    _legend(fig)
    fig.tight_layout(rect=[0, 0.07, 1, 1])
    out = os.path.join(FIG_DIR, "fig_execution_time.pdf")
