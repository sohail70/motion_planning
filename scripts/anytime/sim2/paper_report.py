...
# ===========================================================================================
_BASE = "."   # <-- folder that contains the results_Thruster_* dirs
CONFIGS = {
    "T=35s": f"{_BASE}/results_Thruster_samples_1_time_35.0",
    "T=45s": f"{_BASE}/results_Thruster_samples_1_time_45.0",
    "T=55s": f"{_BASE}/results_Thruster_samples_1_time_55.0",
    "T=65s": f"{_BASE}/results_Thruster_samples_1_time_65.0",
}
CONFIG_ORDER = list(CONFIGS.keys())

CONFIG_FIG_LABEL = {
    "T=35s": r"$T{=}35\,\mathrm{s}$",
    "T=45s": r"$T{=}45\,\mathrm{s}$",
    "T=55s": r"$T{=}55\,\mathrm{s}$",
    "T=65s": r"$T{=}65\,\mathrm{s}$",
}
CONFIG_TXT_LABEL = {
    "T=35s": "T=35s",
    "T=45s": "T=45s",
...
def _repair_panel(ax, tidy, scen):
    ...
    # # Use a linear scale (remove log) and start Y-axis at 0
    # ax.set_ylim(bottom=0)
    # ax.set_xticks(range(nC))
    # ax.set_xticklabels(_cfg_fig_labels(), rotation=18, ha="right", fontsize=8)
    # ax.set_title(scen, fontsize=11)
    # ax.set_ylabel("Repair latency (ms)", fontsize=9)
...
def fig_repair_time(tidy):
    ...
    # Dynamically size the grid: 1 row, N columns (if <= 3 scenarios) or 2x2 (if 4 scenarios)
    n_scen = len(scs)
    if n_scen == 4:
        fig, axes = plt.subplots(2, 2, figsize=(7.6, 7.2), squeeze=False)
    else:
        fig, axes = plt.subplots(1, n_scen, figsize=(3.8 * n_scen, 3.6), squeeze=False)
    ...
    _legend(fig)
    fig.tight_layout(rect=[0, 0.05, 1, 1] if n_scen == 4 else [0, 0.08, 1, 1])
