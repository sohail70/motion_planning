...
std::unique_ptr<Planner> PlannerFactory::createPlanner(PlannerType type, std::unique_ptr<StateSpace> statespace, std::unique_ptr<ProblemDefinition> problem) {  
    auto object = planners_.find(type);
    if (object != planners_.end())
    {
        return object->second(std::move(statespace) , std::move(problem));
    }
