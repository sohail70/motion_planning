...
void KinodynamicFMTX::setup(const Params& params, std::shared_ptr<Visualization> visualization) {
    ...
    if (use_kdtree == true && kdtree_type == "NanoFlann"){
        ...
        weights << 1.0, 1.0, 0.4; // Weights for x, y, theta, time
    ...
    neighborhood_radius_ = 2*factor * gamma * std::pow(std::log(statespace_->getNumStates()) / statespace_->getNumStates(), 1.0 / d);
...
void KinodynamicFMTX::plan() {
    ...
    while (!v_open_heap_.empty() &&
            ...
            if (x->getCost() > cost_via_z) {
                ...
                if (best_parent_for_x != nullptr) {
                    ...
                    if (obstacle_free) {
                        ...
                        v_open_new.push_back(x); //FOR LATER PUSH TO VOPEN (STANDARD FMT* APPROACH USING V_OPNE_NEW) 

                        // INSTANT PUSH TO VOPEN --> this approach might cause the x node to connect to another x node in the same for loop but who cares!
                        if (x->in_queue_) {
        ...
        // for(auto& x : v_open_new){
        //     if (x->in_queue_) {
        //         v_open_heap_.update(x, x->getCost());
        //     } else {
        //         v_open_heap_.add(x, x->getCost()); // add() also sets in_queue_ = true
        //     }
        // }
