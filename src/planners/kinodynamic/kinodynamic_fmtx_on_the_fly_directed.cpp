...
void KinodynamicFMTX::setup(const Params& params, std::shared_ptr<Visualization> visualization) {
    ...
    if (use_rrtx_saved_samples_) {
        ...
        const Eigen::VectorXd& start_state_val = problem_->getStart(); // Destination (time-to-go = 0)
        const Eigen::VectorXd& goal_state_val = problem_->getGoal();   // Robot's initial state

        FMTNode* root_node_ptr = nullptr;

        for (const auto& node_ptr : tree_) {
            if (node_ptr->getStateValue().isApprox(start_state_val)) {
                root_node_ptr = node_ptr.get();
            }
            if (node_ptr->getStateValue().isApprox(goal_state_val)) {
                robot_node_ = node_ptr.get(); // Assign to member variable
            }
        }

        if (!root_node_ptr) throw std::runtime_error("Could not find start state in loaded file.");
        if (!robot_node_) throw std::runtime_error("Could not find goal state in loaded file.");
        ...
        v_open_heap_.add(root_node_ptr, 0.0); // Add root to the open set to begin the search

        // Configure the goal node (the robot's starting position)
        robot_state_index_ = robot_node_->getIndex();
        robot_node_->setTimeToGoal(goal_state_val(2)); // Set its time budget

        std::cout << "Successfully identified start (idx " << root_state_index_
                  << ") and goal (idx " << robot_state_index_ << ") nodes from file.\n";
