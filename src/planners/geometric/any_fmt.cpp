...
ANYFMT::ANYFMT(std::unique_ptr<StateSpace> statespace ,std::shared_ptr<ProblemDefinition> problem_def, std::shared_ptr<ObstacleChecker> obs_checker) :  statespace_(std::move(statespace)), problem_(problem_def), obs_checker_(obs_checker) {
    std::cout<< "ANY FMT Constructor \n";

}

void ANYFMT::clearPlannerState() {
...
void ANYFMT::setup(const Params& params, std::shared_ptr<Visualization> visualization) {
...
void ANYFMT::near(int node_index) {
...
std::vector<Eigen::VectorXd> ANYFMT::getPathPositions() const {
...
void ANYFMT::setRobotIndex(const Eigen::VectorXd& robot_position) {
...
void ANYFMT::setStart(const Eigen::VectorXd& start) {
    ...
    tree_.push_back(std::move(node));
    std::cout << "ANYFMT: Start node created on Index: " << robot_state_index_ << "\n";
}
void ANYFMT::setGoal(const Eigen::VectorXd& goal) {
    ...
    tree_.push_back(std::move(node));
    std::cout << "ANYFMT: Goal node created on Index: " << root_state_index_ << "\n";
}


std::vector<Eigen::VectorXd> ANYFMT::getSmoothedPathPositions(int num_intermediates, int smoothing_passes) const {
...
std::vector<Eigen::VectorXd> ANYFMT::interpolatePath(const std::vector<Eigen::VectorXd>& path, int num_intermediates) const {
...
std::vector<Eigen::VectorXd> ANYFMT::smoothPath(const std::vector<Eigen::VectorXd>& path, int window_size) const {
    // Check for invalid inputs
...
void ANYFMT::visualizePath(std::vector<size_t> path_indices) {
    std::vector<Eigen::VectorXd> nodes;
    std::vector<std::pair<Eigen::VectorXd, Eigen::VectorXd>> edges;
...
void ANYFMT::visualizeSmoothedPath(const std::vector<Eigen::VectorXd>& shortest_path_) {
...
void ANYFMT::visualizeHeapAndUnvisited() {
    std::vector<Eigen::VectorXd> vopen_positions;
