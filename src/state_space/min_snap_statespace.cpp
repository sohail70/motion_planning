...
// Constructor
MinSnapStateSpace::MinSnapStateSpace(int dimension, double v_max, double a_max, const Eigen::VectorXd& w_vel, const Eigen::VectorXd& w_accel, const Eigen::VectorXd& w_snap, unsigned int seed)
...
Trajectory MinSnapStateSpace::steer(const Eigen::VectorXd& from, const Eigen::VectorXd& to) const {
    ...
    for (int axis = 0; axis < num_axes_; ++axis) {
        Eigen::MatrixXd H = calculateCombinedHessian(T, axis);
...
Trajectory MinSnapStateSpace::steer(const Eigen::VectorXd& from, const Eigen::VectorXd& to, const Eigen::VectorXd& v0, const Eigen::VectorXd& a0) const {
    // // This will only print logs when a large turn is requested.
    // double yaw_diff_check = std::abs(normalizeAngle(to(3) - from(3)));
    // if (yaw_diff_check > 1.57) { // 1.57 rad is 90 degrees
    //     std::cout << std::fixed << std::setprecision(3);
    //     std::cout << "\n--- [STEER DEBUG] Large Yaw Change Requested ---" << std::endl;
    //     std::cout << "Time Duration (T): " << (from.tail<1>()[0] - to.tail<1>()[0]) << std::endl;
    //     std::cout << "FROM state yaw: " << from(3) << " | TO state yaw: " << to(3) << std::endl;
    //     std::cout << "PARENT (to) vel_yaw: " << v0(3) << " | acc_yaw: " << a0(3) << std::endl;
    // }
    // // --- END OF DEBUG BLOCK ---
    ...
    // Eigen::MatrixXd H_base = calculateCombinedHessian(T, axis);
    ...
    // // Add this logging at the end of the function to see the output.
    // if (yaw_diff_check > 1.57 && result.is_valid && !result.path_points.empty()) {
    //     std::cout << "CHILD (from) vel_yaw: " << result.final_velocity(3) << " | acc_yaw: " << result.final_acceleration(3) << std::endl;
    //     std::cout << "Generated Yaw Path (Start, Mid, End):" << std::endl;
    //     std::cout << "  - Start: " << result.path_points.front()(3) << std::endl;
    //     std::cout << "  - Mid:   " << result.path_points[result.path_points.size() / 2](3) << std::endl;
    //     std::cout << "  - End:   " << result.path_points.back()(3) << std::endl;
    //     std::cout << "----------------------------------------------\n" << std::endl;
    // }
...
Trajectory MinSnapStateSpace::steer(const Eigen::VectorXd& from, const Eigen::VectorXd& to, const Eigen::VectorXd& v0, const Eigen::VectorXd& a0, const Eigen::VectorXd& v1, const Eigen::VectorXd& a1) const {
    ...
    for (int axis = 0; axis < num_axes_; ++axis) {
        // double effective_w_vel = w_vel_; // Default to the value from the constructor
        // if (axis == 3) {
        //     effective_w_vel = 10.0; // Hardcode a high weight specifically for yaw
        // }
        Eigen::MatrixXd H = calculateCombinedHessian(T, axis);
        // Eigen::MatrixXd H = calculateCombinedHessian(T, effective_w_vel, w_accel_, w_snap_);
        Eigen::VectorXd g = Eigen::VectorXd::Zero(num_coeffs_);
        double start_pos = from(axis);
        double end_pos = to(axis);
        if (axis == 3) { end_pos = start_pos + normalizeAngle(end_pos - start_pos); }
    ...
    // // Add this logging at the end of the function to see the output.
    // if (yaw_diff_check > 1.57 && result_traj.is_valid && !result_traj.path_points.empty()) {
    //     std::cout << "CHILD (from) vel_yaw: " << result_traj.final_velocity(3) << " | acc_yaw: " << result_traj.final_acceleration(3) << std::endl;
    //     std::cout << "Generated Yaw Path (Start, Mid, End):" << std::endl;
    //     std::cout << "  - Start: " << result_traj.path_points.front()(3) << std::endl;
    //     std::cout << "  - Mid:   " << result_traj.path_points[result_traj.path_points.size() / 2](3) << std::endl;
    //     std::cout << "  - End:   " << result_traj.path_points.back()(3) << std::endl;
    //     std::cout << "----------------------------------------------\n" << std::endl;
    // }
...
Eigen::MatrixXd MinSnapStateSpace::calculateCombinedHessian(double T,  int axis) const {
    ...
    double w_accel = w_accel_(axis);
    double w_snap = w_snap_(axis);
    Eigen::MatrixXd H_total = Eigen::MatrixXd::Zero(num_coeffs_, num_coeffs_);
