...
Eigen::VectorXd ThrusterSteerStateSpace::steering1D(double x_start, double x_end, double v_start, double v_end, double t_start, double t_end, double a_max) const {
    ...
    if (v_end >= -EPS) { 
        ...
        if (t_a <= t_b) {
            ...
            double delta_x_min = (t_b - t_a) * v_min_val / 2.0 + tau_1 + tau_2;

            if (delta_x > delta_x_max + EPS || delta_x < delta_x_min - EPS) {
                std::cout << "  DEBUG: Unreachable in this scenario.\n";
                return Eigen::VectorXd::Constant(5, std::numeric_limits<double>::quiet_NaN());
            }
            ...
            if (delta_x < tau_1 + tau_2) {
                ...
                v_coast = v_min_val + z;
                t1 = t_delta_min_v - z / a_max;
                t2 = t_delta_min_v + z / a_max;
                ...
                v_coast = v_max_val - z;
                t1 = t_delta_max_v - z / a_max;
                t2 = t_delta_max_v + z / a_max;
            }
        } else { // Case A, Scenario 2 (t_b < t_a)
            std::cout << "  DEBUG: Case A, Scenario 2 (t_b < t_a) -- MISSING LOGIC WAS HERE.\n";
            ...
            if (delta_x > delta_x_max + EPS) {
                std::cout << "  DEBUG: Unreachable (too high).\n";
                return Eigen::VectorXd::Constant(5, std::numeric_limits<double>::quiet_NaN());
            }

            double delta_x_min = -(t_a - t_b) * v_min_val / 2.0 + tau_1 + tau_2;
            if(delta_x < delta_x_min - EPS) {
                std::cout << "  DEBUG: Unreachable (too low).\n";
                return Eigen::VectorXd::Constant(5, std::numeric_limits<double>::quiet_NaN());
            }
    ...
    // --- Case B: v_end < 0 (after potential flip) ---
    else { 
        std::cout << "  DEBUG: Entering Overall Case B (v_end < 0).\n";
        double tau_1 = (v_start * v_start) / (2.0 * a_max); 
        double t_a = v_start / a_max; 
...
Trajectory ThrusterSteerStateSpace::steer(const Eigen::VectorXd& from, const Eigen::VectorXd& to) const {
    ...
    double actual_duration = from_time - to_time; // This is the total time cost if path is valid
